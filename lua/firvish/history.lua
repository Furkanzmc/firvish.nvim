...
function get_history(predicate)
    ...
    for index, file in ipairs(old_files) do
        if vim.fn.filereadable(file) == 1 and
            (predicate == nil or (predicate ~= nil and predicate(file) == true)) then
...
M.on_buf_delete = function() open_bufnr = nil end

M.on_buf_leave = function() previous_bufnr = nil end
...
M.refresh_history =
    function() utils.set_buf_lines(open_bufnr, get_history(nil)) end

M.open_file = function()
    local linenr = vim.fn.line(".")
    local lines = vim.api.nvim_buf_get_lines(open_bufnr, linenr - 1, linenr,
                                             true)
